...
def run_epoch(session, train_loss, train_opt, valid_loss, valid_index, valid_iterator, valid_data, step, max_length, max_session):
    loss = []
    while True:
        try:
...
def random_name(path):
    ...
    tobigs = tf.global_variables_initializer()
    with tf.Session() as sess:
        sess.run(tobigs)
        # tf.local_variables_initializer().run()
        # tf.global_variables_initializer().run()
        # 초기화
        # train_filenames = tf.global_variables_initializer()
        # print('-'*80)
...
def random_validation(test_path):
    ...
    boaz = tf.global_variables_initializer()
    with tf.Session() as sess:
        sess.run(boaz)
        # tf.local_variables_initializer().run()
        filename = sess.run(train_filenames)
